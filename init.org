...
;; Tip from: https://blog.d46.us/advanced-emacs-startup/
...
(setq w32-pipe-buffer-size 16384)
#+END_SRC
...
Font samples [[file:org/emojis.org][emojis.org]] which contains:
...
*** forward-sexp
Enable slurping hash table literals

#+BEGIN_SRC emacs-lisp
(defun forward-sexp-elisp (arg)
  (let ((target (scan-sexps (point) arg)))
    (when (and target
               (> arg 0)
               (string= "#s"
                        (string-trim-left (buffer-substring (point) target))))
      (setq target (scan-sexps target (cl-signum arg))))
    (when target
      (goto-char target))
    (when (< arg 0)
      (when (lispy-after-string-p "#s")
        (goto-char (- (point) 2)))
      (backward-prefix-chars))))

(defun my-forward-sexp (arg)
  (cond ((derived-mode-p 'emacs-lisp-mode)
         (forward-sexp-elisp arg))
        (t
         ;; Copied from `forward-sexp' in lisp.el
...
(defun shutdown ()
  ...
  (add-hook 'kill-emacs-hook
            (lambda ()
              (call-process "nircmd" nil nil nil "exitwin" "poweroff"))
            t)
  (let ((confirm-kill-emacs nil))
    (save-buffers-kill-emacs '(4))))
...
(defun reboot ()
  ...
  (add-hook 'kill-emacs-hook
            (lambda ()
              (call-process "nircmd" nil nil nil "exitwin" "reboot"))
            t)
  (let ((confirm-kill-emacs nil))
    (save-buffers-kill-emacs '(4))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
;;;###autoload
(defun my-git-diff (dir range)
  (let ((default-directory dir))
    (pop-to-buffer (generate-new-buffer (format "*git diff:%s*" dir)))
    (call-process "git" nil t nil "diff" range)
    (special-mode)
    (goto-char (point-min))))
#+END_SRC
...
(use-package ace-link
  ...
  :after (:or info help-mode eww elfeed)
...
(use-package org
  ...
  :bind (:map org-mode-map
         ("C-c C-r" . nil))             ; reserved for ivy-resume
  ...
  (defun shr--download-image ()
    ...
    (let* ((url (get-text-property (point) 'shr-url))
      ...
      (if image-url
        ...
        (message "WARN: No URL under point."))))
...
(use-package eww
  ...
  (defun ad-eww-display-image (buffer)
    "Add `shr-image-map' to the image."
    (with-current-buffer buffer
      (let ((inhibit-read-only t))
        (add-text-properties (point-min) (point-max)
                             (list 'keymap shr-image-map
                                   'image-url (plist-get eww-data :url))))))

  (advice-add 'eww-display-image :after #'ad-eww-display-image))
...
**** eww-hook
#+BEGIN_SRC emacs-lisp
(use-package eww-hooks
  :after eww
  :custom
  (eww-hooks-readable-alist
   `((,(rx "://kifulog.shogi.or.jp/") . ,(rx bol "RSS"))
     (,(rx "://www.rt.com/news/") . ,(rx bol "HomeWorld News"))
     (,(rx "://github.com/" (+\? graph) "/commit/") . ,(rx bol "Browse files"))
     (,(rx "://www.shogi.or.jp/game/schedule/") . ,(rx bol "対局予定")))))
#+END_SRC
...
(use-package ig-eww
  :defer t
  :config
  (setq ig-eww-video-player #'youtube-dl/play))
#+END_SRC
*** elfeed
#+BEGIN_SRC emacs-lisp
(use-package elfeed
...
(use-package jcom
  ...
  :defer t
  :config
  (setq jcom-program-format-function
        (lambda (prog)
          (let ((default (jcom-program-format-default prog)))
            (let-alist prog
              (if (and (string= "囲碁・将棋チャンネル" .channelName)
                       (string-match-p (rx (or "徹底解説" "銀河戦")) .title))
                  (concat default
                          (format " (%s)"
                                  (cl-find-if (lambda (txt)
                                                (string-match-p "vs" txt))
...
(use-package lispy
  ...
  (delight 'lispy-mode '(:eval (lispy-visualize-mode)) 'lispy)
...
(use-package flycheck
  ...
  (advice-add 'flycheck-relevant-error-other-file-p :override #'ignore)
  ...
  (defvar flycomp-mode-alist
    '((emacs-lisp-mode . byte-recompile-file)))

  (defun flycomp (status)
    (when (and (eq status 'finished)
               (not (buffer-modified-p))
               (null (flycheck-count-errors flycheck-current-errors)))
      (when-let ((fn (alist-get major-mode flycomp-mode-alist)))
        (funcall fn  buffer-file-name))))

  (add-hook 'flycheck-status-changed-functions #'flycomp)
...
(use-package delay
  :after lispy
  :config
  (defun ad-lispy--eval-elisp-promise (oldfun e-str)
    (let ((e-sexp (read e-str)))
      (if (promise-p e-sexp)
          (prin1-to-string (force e-sexp))
        (funcall oldfun e-str))))
  (advice-add 'lispy--eval-elisp :around #'ad-lispy--eval-elisp-promise))
#+END_SRC
  ...
  (with-eval-after-load 'browse-url
    (add-to-list 'browse-url-browser-function
                 (cons (rx bow (or "dailymotion.com"
                       ...
                       'youtube-dl/play)))
  :config
  (setq youtube-dl-command/play "mpv --force-window=yes --ytdl-raw-options=netrc=,mark-watched= %s"
...
(use-package ffedit
  ...
  (setq ffedit-find-video-function #'ffedit-find-video/ivy
        ...
        ytdl-format/hd (eval-when-compile
                         (string-join
                          '("(bestvideo[ext=mp4]/bestvideo[ext=webm])[height<=?720][protocol!=http_dash_segments]+(bestaudio[ext=m4a]/bestaudio[ext=webm])[protocol!=http_dash_segments]"
...
*** which-func-mode
Only for making characters visible.
#+BEGIN_SRC emacs-lisp
